...
KeyEvent KeyboardClient::ReadKey()
{
	...
	{
		return KeyEvent(KeyEvent::EventType::Invalid, 0);
	}
}
...
void KeyboardServer::OnKeyPressed(unsigned char keycode)
{
	...
	keybuffer.push(KeyEvent(KeyEvent::EventType::Press, keycode));
...
void KeyboardServer::OnKeyReleased(unsigned char keycode)
{
	...
	keybuffer.push(KeyEvent(KeyEvent::EventType::Release, keycode));
	if (keybuffer.size() > bufferSize)
	{
		keybuffer.pop();
	}
