...
std::vector<double> rgb_to_hsv(const CColor& c)
{
	double r = c.GetR();
	double g = c.GetG();
	double b = c.GetB();

	double h = 0.0, s = 0.0, v = std::max({ r,g,b });
...
CColor hsv_to_rgb(const std::vector<double>& hsv)
{
	double r, g, b;
	...
	return CColors::MakeRGB((unsigned int)std::round(r), (unsigned int)std::round(g), (unsigned int)std::round(b));
}

CColor interpolation(const CColor& a, const CColor& b, double factor)
{
...
std::vector<CColor> generate_colors(const CColor& start, const CColor& end, int number_of_colors)
{
	std::vector<CColor> res;
	for (int i = 0; i < number_of_colors; i++)
	{
		CColor rgb = interpolation(start, end, (double)i / (std::max(number_of_colors, 2) - 1));
		res.push_back(rgb);
	}
	return res;
}
